...
class ClassVisualization:
    ...
    def create_class_visualization(self, target_y, class_names, model, dtype, **kwargs):
        ...
        for t in range(num_iterations):
            ...
            result_f = model.forward(img_var)

            model.zero_grad()
            result_f[0, target_y].backward()

            dimg_var = (img_var.grad.data) / (((img_var.grad.data**2).sum()).sqrt() + 1e-11)
            img_var.grad.data.zero_()
            img_var.data += ((learning_rate*dimg_var) - l2_reg*(2*img_var.data))
